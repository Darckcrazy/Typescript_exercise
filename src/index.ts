...
const concatenaEConteggia = (str1: string, str2: string): number => {
...
let numeroOpzionale: number | null | undefined = 42;
...
// 9) Tipo per giorni della settimana
...
// 11) Tupla
const tupla: [string, string, string, number, number] = ["primo", "secondo", "terzo", 10, 20];
...
// 13) Interfaccia per oggetto con firstname, lastname, age
interface Persona {
    firstname: string;
    lastname: string;
    age: number;
...
// 14) Interfaccia utente con email obbligatoria e telefono opzionale
interface Utente {
    email: string;
